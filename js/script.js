...
document.getElementById('validateBtn').addEventListener('click', () => {
    const transId = document.getElementById('transactionIdInput').value;
    const message = document.getElementById('validationMessage');
    
    if (!transId.trim()) {
        showValidationMessage('Please enter a transaction ID', 'error');
        return;
    }
    
    const isValid = validateTransactionID(transId);
    
    if (isValid) {
        showValidationMessage('✓ Payment verified successfully!', 'success');
        setTimeout(() => {
        ...
        showValidationMessage('✗ ACCESS DENIED - Invalid transaction ID or amount', 'error');
    }
});
...
function validateTransactionID(transId) {
    const amount = paymentType === 'download' ? 600 : 240;
    const today = new Date().toISOString().split('T')[0];
    
    // Check for required patterns
    const hasDate = transId.includes(today);
    const hasAmount = transId.includes('K' + amount) || transId.includes(amount.toString());
    const hasAccountName = transId.includes('Abraham Msofi');
    const hasAccountNumber = transId.includes('1011288266') || 
                           transId.includes('+265993984344') || 
                           transId.includes('+265897228943');
    
    return hasDate && hasAmount && hasAccountName && hasAccountNumber;
}
